...
CREATE TABLE `ecoturismo_test`.`usuarios` (
...
#tabla perfiles
CREATE TABLE `ecoturismo_test`.`perfiles` (
    `id_usuario` INT NOT NULL AUTO_INCREMENT,
...
#tabla macroregiones
CREATE TABLE `ecoturismo_test`.`macroregiones` (
    `id_macroregion` INT NOT NULL AUTO_INCREMENT,
    `macroregion` VARCHAR(255) NOT NULL,
    PRIMARY KEY (`id_macroregion`));

#tabla regiones
CREATE TABLE `ecoturismo_test`.`regiones` (
    `id_region` INT NOT NULL AUTO_INCREMENT,
    `region` VARCHAR(255) NOT NULL,
    PRIMARY KEY (`id_region`));

#tabla ubicaciones
CREATE TABLE `ecoturismo_test`.`ubicaciones` (
    ...
    CONSTRAINT `fk_ubicacion_macroregion`
        ...
        REFERENCES `ecoturismo_test`.`macroregiones` (`id_macroregion`)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT `fk_ubicacion_region`
        FOREIGN KEY (`id_region`)
        REFERENCES `ecoturismo_test`.`regiones` (`id_region`)
        ON DELETE CASCADE
        ON UPDATE CASCADE);

#tabla destinos
CREATE TABLE `ecoturismo_test`.`destinos` (
    ...
    CONSTRAINT `fk_destino_ubicacion`
        ...
        REFERENCES `ecoturismo_test`.`ubicaciones` (`id_ubicacion`)
        ON DELETE CASCADE
        ON UPDATE CASCADE);

#tabla recomendaciones
CREATE TABLE `ecoturismo_test`.`recomendaciones` (
    ...
    CONSTRAINT `fk_recomendacion_destino`
        ...
        REFERENCES `ecoturismo_test`.`destinos` (`id_destino`)
        ON DELETE CASCADE
        ON UPDATE CASCADE);

#tabla comentarios
CREATE TABLE `ecoturismo_test`.`comentarios` (
    ...
    CONSTRAINT `fk_comentario_usuario`
        FOREIGN KEY (`id_usuario`)
        REFERENCES `ecoturismo_test`.`perfiles` (`id_usuario`)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
